...
namespace Trasua.Controllers
{
    public class CheckOutController : Controller
    {
        ...
        public CheckOutController(TrasuaContext context, IConfiguration  config )
        {
            _context = context;
        ...
        public IActionResult CheckoutSuccess()
        {
            ...
            {
                TempData["error"] = "Thiếu thông tin khách hàng trong phiên làm việc.";
                return RedirectToAction("Index", "Cart");
            }
            ...
            {
                CartItems = cartItems,
                GrandTotal = (double)cartItems.Sum(x => x.SoLuong * (x.GiaKm ?? x.GiaBan))
            };
            ...
            try
            {
                ...
                foreach (var cart in cartItems)
                {
                    var orderDetail = new ChiTietHd
                    {
                        MaHd = hoaDon.MaHd,
                        MaSp = cart.MaSp,
                        Price = (decimal)(cart.GiaKm ?? cart.GiaBan),
            ...
            catch (DbUpdateException ex)
            {
                ...
                // Ghi lại lỗi (cân nhắc sử dụng một framework ghi log như Serilog hoặc NLog)
                Console.Error.WriteLine(innerException?.Message);
                TempData["error"] = "Có lỗi xảy ra trong quá trình thanh toán. Vui lòng thử lại.";
                return RedirectToAction("Index", "Cart");
            }
        }
        public IActionResult Lichsu(int page = 1, int pageSize = 10)
        {
